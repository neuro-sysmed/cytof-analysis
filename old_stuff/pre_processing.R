...
# not been through other algorithms; directly from the XT (_prosessed.fcs). However, doing 
...
double_delims = stri_detect_fixed(unlist(ff@description), "\\\\") #check the whole header for double delims
...
sce <- prepData('data/renamed/', transform = TRUE) #reads the data in sce
table(sce$sample_id)
...
key  = sample_key # We need integer data here. 
...
plotYields(sce, out_path = getwd(), out_name = "Barcode_yields_allsamples")
plotYields(sce, which = rownames(key), out_path = getwd(), out_name = "Barcode_yields_Eachsample")
table(sce$sample_id)
...
write.flowSet(fs, outdir = paste0(data_dir,"/debarcoded/"))
...
sce <- runDR(sce, dr = "UMAP", cells = 5000, features = "state") # cells is per sample!
